...
///
class BackPanel11 extends BackPanel {
  BackPanel11({super.key});
  ...
  Widget buildBackPanelPortrait(
    ...
    children: [
      ...
      screen.box(
        Rect.fromLTWH(screen.width - 0.8, 0.0, 0.8, 0.8),
  ...
  Widget buildBackPanelLandscape(
    ...
    children: [
      Container(color: MainScreen.keyboardBaseColor),
      screen.box(
        const Rect.fromLTWH(11.8, 0.4, 0.8, 0.8),
        const Icon(Icons.arrow_back, color: Colors.white),
      ),
      screen.box(
  ...
  List<Widget> jumpTableList(ScreenPositioner screen, double x, double y) => [
    ...
    screen.box(
      Rect.fromLTWH(x - 3.49 + 4.05, y + 1.1, 1.5, 3),
      jumpTableKey('\u200a\u200aISG\u200a', 1.5 * 1.2),
    ),
    screen.box(
      Rect.fromLTWH(x - 3.49 + 5.6, y + 1.1, 1.5, 3),
      jumpTableKey('DSE', 1.5 * 1.2),
    ),
  ];

  Widget sigmaTable(double widthCM) => table(widthCM, [
    row([
      cell(space(1)),
  ...
  Widget jumpTable(double widthCM, double heightCM) => _JumpTable(
    widthCM * 10,
    heightCM * 10,
    this,
    topText: cell(
  ...
  Widget metricTable(double widthCM) => table(widthCM, [
    row([cell(text(' cm \u00f7 2.54')), cell(arrowRight(2)), cell(text('in'))]),
    row([
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int j = 0; j < 2; j++) {
      ...
      for (int i = 0; i < 4; i++) {
        ...
        canvas.drawArc(
          const Rect.fromLTWH(10.95, 9.95, 4.5, 4.5),
          i * dashAdvance + 3 * dart.pi / 2.0,
          dashLen,
          false,
          thinLine,
        );
      ...
      canvas.drawLine(
        const Offset(13.2, 14.5),
        const Offset(13.2, 18.1),
        thinLine,
      );
      for (int i = 0; i < 2; i++) {
        const sz = 0.6;
